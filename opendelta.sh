...
BIN_JAVA=java
PATH_TOOLS=$(cd `dirname $0`; pwd)
PATH_OD=$(dirname "${PATH_TOOLS}")
BIN_MINSIGNAPK=$PATH_TOOLS/minsignapk.jar
BIN_XDELTA=$PATH_TOOLS/xdelta3
BIN_ZIPADJUST=$PATH_TOOLS/zipadjust

FILE_MATCH=Nitrogen-*.zip
PATH_CURRENT=/root/nitrogen/out/target/product/$DEVICE
PATH_LAST=$PATH_OD/last/$DEVICE

KEY_X509=$PATH_TOOLS/keys/platform.x509.pem
KEY_PK8=$PATH_TOOLS/keys/platform.pk8
...
echo "}" >> $DELTA
pwd
mkdir  ~/onedrive/sagit/publish >/dev/null 2>/dev/null
mkdir ~/onedrive/sagit/publish/$DEVICE >/dev/null 2>/dev/null
cp out/* ~/onedrive/sagit/publish/$DEVICE/.
